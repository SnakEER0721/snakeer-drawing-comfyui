...
SILENT_ALLOWED = {
    ...
    ("_port_free", 0):            "端口被占用正是这个函数要检测的情况，"
                                  "bind 失败就是答案，不是异常",
    # check_param_ranges 里这两处 continue 都是**故意的**，而且都有一条
    # 兜底路径接住，不是"吞掉就算了"：
    #   ① 值不是数字（None/list/dict 等）→ float() 抛 TypeError。这些值到不了
    #      建图函数：`_read_body` 的 drop_null_params 已经把 null 删掉，
    #      剩下的真到下游会由 `int()`/`float()` 抛出一句更贴切的 BadParam。
    #   ② 值是字符串 → 上面就跳过了，走不到这里；走到这里的是类型没覆盖到的
    #      稀有情况，同样交给下游。
    # 为什么不在这里直接报：这里只能给出"应该是数字"这种笼统的话；
    # 下游知道**这个参数该是整数还是浮点**（`num_param` 的 cast 参数），
    # 报错信息质量差一档 —— 而"报错要能照着改"是这个项目的硬规矩。
    ("check_param_ranges", 0):    "值不是数字时交给下游 int()/float() 报更贴切的错",
    ("check_param_ranges", 1):    "同上（PARAM_FINITE_ONLY 那一轮）",
